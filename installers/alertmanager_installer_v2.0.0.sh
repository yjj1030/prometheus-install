...
# 用户实际路由模板请参考 src/v6r4-RC6/config/alertmanager/alertmanager.yml
...
Group=${ALERTMANAGER_GROUP}"
  ...
  if [[ "${PLATFORM_ID}" == "rhel" ]]; then
    # RC7: ProtectSystem=full 会使 /usr/local 只读；仅放行 Alertmanager
    # 运行期需要写入的 data 与 logs，避免 Kylin/RHEL-family 启动失败。
...
# 安全校验组件根目录下的状态目录路径。
# RC6 安全修复：禁止 state 目录或其任一中间目录通过符号链接逃逸到 INSTALL_ROOT 之外。
# 仅用于 installer 自身 state/ownership/access-request 等状态文件路径。
secure_validate_dir_under_root() {
  local target="$1" base="$2" rb rt rel cur comp
  [[ -n "${target}" && -n "${base}" ]] || { echo "[state] 拒绝空路径" >&2; return 1; }
  [[ "${target}" == /* && "${base}" == /* ]] || { echo "[state] 仅允许绝对路径" >&2; return 1; }

  base="${base%/}"
  [[ "${base}" != "" && "${base}" != "/" ]] || { echo "[state] 非法组件根目录: ${base:-/}" >&2; return 1; }
  [[ "${target}" == "${base}/"* ]] || { echo "[state] 路径不在组件根目录内: ${target}" >&2; return 1; }

  # RC6 first-install regression fix:
  # - base 不存在：不是安全失败，而是“可安全创建”状态（return 2）。
  # - base 已存在但为 symlink / 非目录：fail closed。
  # 先检查 -L，确保 broken symlink 也不会被误判为“不存在”。
  if [[ -L "${base}" ]]; then
    echo "[state] 安全保护：组件根目录不能是符号链接: ${base}" >&2
    return 1
  fi
  if [[ ! -e "${base}" ]]; then
    return 2
  fi
  [[ -d "${base}" ]] || { echo "[state] 安全保护：组件根目录不是目录: ${base}" >&2; return 1; }

  rb="$(readlink -f -- "${base}" 2>/dev/null)" || { echo "[state] 无法解析组件根目录: ${base}" >&2; return 1; }
  rel="${target#${base}/}"
  [[ -n "${rel}" ]] || { echo "[state] 拒绝把组件根目录作为状态目录" >&2; return 1; }

  cur="${base}"
  IFS='/' read -r -a _state_parts <<< "${rel}"
  for comp in "${_state_parts[@]}"; do
    [[ -n "${comp}" && "${comp}" != "." && "${comp}" != ".." ]] || {
      echo "[state] 非法路径组件: ${target}" >&2
      return 1
    }
    cur="${cur}/${comp}"
    [[ ! -L "${cur}" ]] || {
      echo "[state] 安全保护：检测到符号链接目录，拒绝访问: ${cur}" >&2
      return 1
    }
    if [[ -e "${cur}" && ! -d "${cur}" ]]; then
      echo "[state] 安全保护：状态路径不是目录: ${cur}" >&2
      return 1
    fi
  done

  [[ -d "${target}" ]] || return 2
  rt="$(readlink -f -- "${target}" 2>/dev/null)" || return 1
  [[ "${rt}" == "${rb}/"* ]] || {
    echo "[state] 安全保护：状态目录真实路径越界: ${rt}" >&2
    return 1
  }
  return 0
}

secure_ensure_dir_under_root() {
  local target="$1" base="$2" mode="${3:-0755}" owner="${4:-root}" group="${5:-root}"
  local rc rel cur comp rb rt parent

  # RC6: 首次安装时 INSTALL_ROOT 合法但尚不存在，由安全层负责创建。
  # 不使用 mkdir -p 绕过校验：只创建最终 component root；其父目录必须已经存在。
  # 如果 base 已存在为 symlink（含 broken symlink）则明确拒绝。
  base="${base%/}"
  if [[ -L "${base}" ]]; then
    echo "[state] 安全保护：组件根目录不能是符号链接: ${base}" >&2
    return 1
  ...
  if [[ ! -e "${base}" ]]; then
    parent="$(dirname -- "${base}")"
    [[ -d "${parent}" ]] || {
      echo "[state] 组件根目录的父目录不存在，拒绝隐式递归创建: ${parent}" >&2
      return 1
    }
    mkdir -m 0755 -- "${base}" || return 1
    [[ -d "${base}" && ! -L "${base}" ]] || {
      echo "[state] 安全保护：组件根目录创建后类型异常: ${base}" >&2
      return 1
    }
    chown root:root "${base}" 2>/dev/null || true
  fi

  secure_validate_dir_under_root "${target}" "${base}" && {
  ...
  base="${base%/}"
  rb="$(readlink -f -- "${base}" 2>/dev/null)" || return 1
  rel="${target#${base}/}"
  cur="${base}"
  IFS='/' read -r -a _state_parts <<< "${rel}"
  for comp in "${_state_parts[@]}"; do
    ...
    cur="${cur}/${comp}"
    [[ ! -L "${cur}" ]] || {
      echo "[state] 安全保护：检测到符号链接目录，拒绝创建: ${cur}" >&2
      return 1
    }
    if [[ -e "${cur}" ]]; then
      [[ -d "${cur}" ]] || { echo "[state] 状态路径不是目录: ${cur}" >&2; return 1; }
    else
      mkdir -m "${mode}" -- "${cur}" || return 1
    fi
  done

  rt="$(readlink -f -- "${target}" 2>/dev/null)" || return 1
  [[ "${rt}" == "${rb}/"* ]] || {
    echo "[state] 安全保护：新建状态目录真实路径越界: ${rt}" >&2
    return 1
  }
  chmod "${mode}" "${target}" 2>/dev/null || true
  chown "${owner}:${group}" "${target}" 2>/dev/null || true
...
sha256_check() {
  ...
  [[ -n "${expect}" ]] || { echo "sha256_check: 未提供期望 SHA256" >&2; return 64; }
  command -v sha256sum >/dev/null 2>&1 || { echo "sha256_check: 缺少 sha256sum" >&2; return 127; }
  local actual
...
acl_emit() {
  ...
  secure_ensure_dir_under_root "$(dirname "${out}")" "${INST_ROOT:-/tmp}" 0755 root root || return 1
  {
    echo "# 访问申请清单（自动生成）"
    echo
...
ownership_register() {
  ...
  secure_ensure_dir_under_root "${state_dir}" "${INST_ROOT:-/tmp}" 0755 root root || return 1
...
ownership_forget() {
  ...
  secure_validate_dir_under_root "${state_dir}" "${INST_ROOT:-/tmp}" || return 1
  [[ -f "${out}" ]] || return 0
  local tmp
  tmp="$(mktemp)"
  awk -F'\t' -v id="${instance_id}" '$1 != id' "${out}" > "${tmp}" || true
...
ownership_list() {
  ...
  local state_dir
  state_dir="$(dirname "${out}")"
  if [[ -e "${state_dir}" || -L "${state_dir}" ]]; then
    secure_validate_dir_under_root "${state_dir}" "${INST_ROOT:-/tmp}" || { echo "(invalid state path)"; return 1; }
  fi
  if [[ -f "${out}" ]]; then
...
# 安全删除实例级 state：仅允许删除 ${INSTALL_ROOT}/state/<instance-id>，
# 绝不删除 state 根目录、组件根目录或其他实例 state。
# RC4: full uninstall 清理 installer 自身产生的 access-request.md 等实例级状态；
# preserve 模式不得调用本函数。
safe_remove_instance_state_dir() {
  local target="$1" install_root="$2" lexical rb state_root rt
  [[ -n "${target}" && -n "${install_root}" ]] || { echo "[uninstall] 拒绝删除空 state 路径" >&2; return 1; }

  # 第一层：词法边界。只允许 INSTALL_ROOT/state 的直接子目录。
  lexical="$(readlink -m -- "${target}")"
  rb="$(readlink -f -- "${install_root}" 2>/dev/null || readlink -m -- "${install_root}")"
  state_root="${rb}/state"
  [[ "${lexical}" != "/" && "$(dirname -- "${lexical}")" == "${state_root}" ]] || {
    echo "[uninstall] 安全保护：拒绝删除非实例级 state 路径: ${lexical}" >&2
    return 1
  }

  # state 不存在时没有清理动作；不为清理而创建目录。
  [[ -e "${state_root}" ]] || { echo "[uninstall] state 根目录不存在，无需清理: ${state_root}"; return 0; }

  # 第二层：symlink/真实路径边界。state 根必须真实位于组件根下；目标本身不得是 symlink。
  [[ ! -L "${state_root}" ]] || { echo "[uninstall] 安全保护：state 根目录是符号链接，拒绝删除" >&2; return 1; }
  [[ ! -L "${target}" ]] || { echo "[uninstall] 安全保护：实例 state 是符号链接，拒绝删除: ${target}" >&2; return 1; }
  [[ -e "${target}" ]] || { echo "[uninstall] 实例 state 不存在，无需删除: ${target}"; return 0; }
  rt="$(readlink -f -- "${target}")"
  [[ "$(dirname -- "${rt}")" == "${state_root}" ]] || {
    echo "[uninstall] 安全保护：实例 state 真实路径越界: ${rt}" >&2
    return 1
  }

  rm -rf --one-file-system -- "${target}"
  echo "[uninstall] 已删除实例 state: ${target}"
}
...
# 非交互/CLI 安装同样必须执行端口占用门禁，不能只依赖交互向导。
# 必须在创建用户、目录、ACL、systemd unit 等持久化动作之前调用。
require_install_port_free() {
  local port="$1" label="${2:-监听端口}" recommended=""
  if ! [[ "${port}" =~ ^[0-9]+$ ]] || (( port < 1 || port > 65535 )); then
    echo "[ERROR] ${label} ${port} 非法；必须为 1-65535。" >&2
    return 1
  fi
  if port_in_use "${port}"; then
    recommended="$(next_free_port "$((port+1))" "${3:-}" || true)"
    echo "[ERROR] ${label} ${port} 已被占用；安装已停止，禁止复用监听中的端口。" >&2
    [[ -n "${recommended}" ]] && echo "        建议改用空闲端口: ${recommended}" >&2
    return 1
  fi
  return 0
}
...
# 托管实例的独立存活探测：与 status 分离，验证 systemd、PID、端口和 HTTP 探针。
managed_instance_probe() {
  local prefix="$1" unit="${SELECTED_INSTANCE_UNIT:-}" port="${SELECTED_INSTANCE_PORT:-}"
  local pid="0" path="/metrics" code="000" failed=0
  [[ -n "${unit}" ]] || unit="$(service_unit)"
  [[ -n "${port}" ]] || port="$(_detect_unit_port "${unit}" "${prefix}")"

  echo "[probe] unit=${unit} port=${port:--}"
  ...
  if [[ "${pid}" =~ ^[1-9][0-9]*$ ]] && kill -0 "${pid}" 2>/dev/null; then
    ...
    echo "  process : NOT alive (pid=${pid:-0})"
    failed=1
  fi
  ...
  case "${prefix}" in
    prometheus|alertmanager) path="/-/ready" ;;
    grafana) path="/api/health" ;;
    victoriametrics) path="/health" ;;
    *) path="/metrics" ;;
  esac
  if [[ -n "${port}" ]] && command -v curl >/dev/null 2>&1; then
    code="$(curl --noproxy '*' --connect-timeout 2 --max-time 5 -s -o /dev/null -w '%{http_code}' "http://127.0.0.1:${port}${path}" 2>/dev/null || true)"
    [[ -n "${code}" ]] || code=000
    case "${code}" in
      2??|3??) echo "  http    : ${path} HTTP ${code}" ;;
      401|403) echo "  http    : ${path} HTTP ${code} (服务可达，需要认证)" ;;
      *) echo "  http    : ${path} HTTP ${code} (异常)"; failed=1 ;;
    esac
  fi
...
external_instance_uninstall() {
  ...
  if [[ -n "${inferred_root}" ]]; then
    echo "    1) 完整卸载 [默认] — stop+disable、删除 unit，并删除检测到的实例目录 ${inferred_root}"
    echo "    2) 仅移除服务 — 删除 unit，但保留程序/配置/数据"
  else
    echo "    1) 仅移除服务 [默认] — stop+disable 并删除 unit；因无法安全识别实例目录，不删除程序/配置/数据"
    ...
    case "${choice}" in
      ...
      2|3|4|5)
        ...
        if [[ "${SELECTED_INSTANCE_TYPE}" == managed ]]; then
          case "${choice}" in
            2) cmd_status ;;
            3) managed_instance_probe "${prefix}" ;;
            4) cmd_reload ;;
            5)
