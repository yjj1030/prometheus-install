...
stop_srv() {
  ...
  wait "${pid}" 2>/dev/null || true
  # 进程已回收后必须从 SRV_PIDS 删除，避免后续 PID 复用导致 EXIT cleanup 误杀/误等其他进程。
  local -a kept=()
  local p
...
start_srv_checked() {
  local mode="$1" delay="${2:-0}" attempt port pid
  for attempt in 1 2 3 4 5; do
    port="$(pick_port)"
    start_srv "${mode}" "${port}" "${delay}"
    pid="${STARTED_PID}"
    ...
    echo "WARN: ${mode} server failed to listen on ${port}; retry ${attempt}/5" >&2
    stop_srv "${pid}" || true
  done
  echo "FATAL: ${mode} server did not start after retries" >&2
  return 99
}
...
run_ready_bounded() {
  ...
  # 使用独立 runner 文件，而不是 export -f 后 bash -c 调用。
  # 原 export -f 方式在部分 bash/timeout 组合下出现过 Grafana 503 场景挂起；
  # runner 文件能把被测函数、环境变量和执行入口固定下来，便于 timeout 正确管理子进程。
  local runner
  runner="$(mktemp -t t2runner.XXXXXX.sh)"
  {
    declare -f wait_for_prometheus_ready
    declare -f wait_for_grafana_ready
    if [[ "${svc}" == "prom" ]]; then
      echo 'wait_for_prometheus_ready'
    else
      echo 'wait_for_grafana_ready'
    fi
  } >"${runner}"
  chmod 700 "${runner}"
  if [[ "${svc}" == "prom" ]]; then
    PORT="${port}" WAIT_PROM_READY_TIMEOUT="${deadline}" \
      timeout --signal=TERM --kill-after=2 "${hard_limit}s" \
      bash "${runner}" >/tmp/t2-stdout 2>/tmp/t2-stderr
    rc=$?
  else
    PORT="${port}" WAIT_GRAFANA_READY_TIMEOUT="${deadline}" \
      timeout --signal=TERM --kill-after=2 "${hard_limit}s" \
      bash "${runner}" >/tmp/t2-stdout 2>/tmp/t2-stderr
    rc=$?
  fi
...
for SVC in prom grafana; do
  ...
  start_srv_checked "slow200" "2.5" || exit $?
  PORT_X="${STARTED_PORT}"; PID="${STARTED_PID}"
...
start_srv_checked "200" || exit $?
AUTH_PORT="${STARTED_PORT}"; PID="${STARTED_PID}"
echo "----------------------------------------------------------"
