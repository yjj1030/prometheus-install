...
render_scrape_configs() {
  ...
  if [[ ! -d "${SCRAPE_CONFIGS_SRC}" ]]; then
    ...
    if sed -n '/^# __SCRAPE_NODE_EMBED_BEGIN__$/,/^# __SCRAPE_NODE_EMBED_END__$/p' "${BASH_SOURCE[0]}" \
      ...
      echo "[render_scrape_configs] 用户需手动从 src/v6r4-RC4/config/prometheus/scrape_configs/*.yml 拷贝到 ${inst}/etc/scrape_configs/ 并替换占位符" >&2
      return 0
    fi
  fi
...
# 实际告警规则请从 src/v6r4-RC4/config/prometheus/rules/*.yml 拷贝并按需启用。
groups:
