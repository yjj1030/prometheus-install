...
source "${COMMON}"
python3 - <<'PY2' >/tmp/rc3-port-test.out 2>/dev/null &
import socket,time
s=socket.socket(); s.bind(('127.0.0.1',0)); print(s.getsockname()[1], flush=True); s.listen(1); time.sleep(20)
PY2
pid=$!
for _ in $(seq 1 50); do [[ -s /tmp/rc3-port-test.out ]] && break; sleep 0.05; done
port="$(cat /tmp/rc3-port-test.out)"
if port_in_use "${port}"; then ok "port_in_use detects live listener"; else bad "port_in_use missed live listener"; fi
if require_install_port_free "${port}" "test" >/tmp/rc3-port-gate.out 2>&1; then bad "require_install_port_free allowed occupied port"; else ok "CLI gate rejects occupied port"; fi
kill "${pid}" 2>/dev/null || true; wait "${pid}" 2>/dev/null || true; rm -f /tmp/rc3-port-test.out /tmp/rc3-port-gate.out
...
if grep -q 'DEFAULT_VERSION="1.151.0"' "${VM}"; then ok "VictoriaMetrics security baseline is 1.151.0"; else bad "VictoriaMetrics default version is below RC3 security baseline"; fi
