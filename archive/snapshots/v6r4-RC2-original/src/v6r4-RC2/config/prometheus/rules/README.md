...
v3 告警 / 记录规则文件。本目录下的所有规则均由 `prometheus_installer_v6.0.0.sh`
按 `rule_files: - '<inst>/etc/rules/*.yml` 加载；center 端由 `vmalert` 通过
`-rule=<rules_dir>/*.yml` 加载。
...
| `global_alerts_v1.0.0.yml` | 跨站点 / SLO burn-rate / 数据平面 | center (vmalert) | 中心评估 |
