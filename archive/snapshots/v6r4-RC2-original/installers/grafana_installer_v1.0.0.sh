...
# 上游 fallback：dl.grafana.com/oss/release/grafana-${VERSION}.linux-amd64.tar.gz
# ------------------------------------------------------------------------------
download_grafana() {
...
wizard_install() {
  ...
  while true; do
    ...
    ui_ask GRAFANA_BUILD_ID "Grafana build id" "${GRAFANA_BUILD_ID}" "仅用于内网镜像文件名" || return 1
    ...
    upstream="https://dl.grafana.com/oss/release/grafana-${GRAFANA_VERSION}.linux-amd64.tar.gz"
